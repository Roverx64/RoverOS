...
EFI_STATUS openFile(EFIFile *file, uint16 *name, uint64 mode, uint64 flags){
    ...
    if(status == EFI_NOT_FOUND){Print(L"Failed to find file\n"); return status;}
    if(status == EFI_NO_MEDIA){Print(L"No medium found\n"); return status;}
    if(status == EFI_OUT_OF_RESOURCES){Print(L"Not enough resources to open file\n"); return status;}
    return EFI_SUCCESS;
}
