...
EFI_STATUS loadVolume(EFIFile *file){
...
EFI_STATUS loadFile(EFIFile *file){
    uefi_call_wrapper(file->Volume->Open,5,file->Volume,&file->File,file->name,file->mode,file->flags);
    return EFI_SUCCESS;
}