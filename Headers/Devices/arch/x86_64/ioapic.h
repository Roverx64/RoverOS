...
struct ioapicInfo{
    struct ioapicMMIO *mmio; //Virtual address
    uint64_t physBAR; //Physical address
    uint32_t intmap; //Bitmap of allocated interrupts
};
...
extern uint8_t allocateIOAPICInterrupt(uint8_t vector, uint8_t mode, uint8_t apicid);